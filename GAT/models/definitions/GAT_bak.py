...
class GATLayerImp3(GATLayer):
    ...
    def forward(self, data):
        ...
        in_nodes_features, edge_index, gates= data  # unpack data
        ...
        # shape = (E, NH, 1)
        attentions_per_edge = self.neighborhood_aware_softmax(scores_per_edge, edge_index[self.trg_nodes_dim], num_of_nodes,gates)
        ...
        return (out_nodes_features, edge_index,gates)

    #
    # Helper functions (without comments there is very little code so don't be scared!)
    #

    def neighborhood_aware_softmax(self, scores_per_edge, trg_index, num_of_nodes,gates):
        """
        ...
        if gates is not None:
            exp_scores_per_edge = exp_scores_per_edge * gates
        # print(exp_scores_per_edge)
...
# edge_index =torch.load('edge_index.pkl').to('cpu')
# mask=np.load('mask.npy')
# mask_te = np.load('mask_te.npy')
# node_names = np.load('node_names.npy', allow_pickle=True)
# x=torch.load('x.pkl').to('cpu')
# y=np.load('y.npy')
# y_te = np.load('y_te.npy')
# data_CPDB = Data(x=x,edge_index=edge_index,mask=mask,mask_te=mask_te,node_names=node_names,y=y,y_te=y_te)
# torch.save(data_CPDB,'./data_CPDB.pkl')
